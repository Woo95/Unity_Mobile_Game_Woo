...
public static class Gizmos2
{
	...
	public static void DrawCircle2(Vector3 position, Vector3 up, Color color, float radius = 1.0f)
	{
		up = ((up == Vector3.zero) ? Vector3.up : up).normalized * radius;
		Vector3 _forward = Vector3.Slerp(up, -up, 0.5f);
		Vector3 _right = Vector3.Cross(up, _forward).normalized * radius;
		...
		Vector3 _lastPoint = position + matrix.MultiplyPoint3x4(new Vector3(Mathf.Cos(0), Mathf.Sin(0), 0));
		Vector3 _nextPoint = Vector3.zero;
		...
		{
			_nextPoint.x = Mathf.Cos((i * 4) * Mathf.Deg2Rad);
			_nextPoint.y = Mathf.Sin((i * 4) * Mathf.Deg2Rad);
			_nextPoint.z = 0;
	...
	public static void DrawCircle2(Vector3 position, Color color, float radius = 1.0f)
	{
		DrawCircle2(position, Vector3.up, color, radius);
	}
	...
	public static void DrawCircle2(Vector3 position, float radius = 1.0f)
	{
		DrawCircle2(position, Vector3.up, Color.white, radius);
	}
